...
public class BundleDeployHelperTest {
	...
	public void testBundleDeployHelper_response200() throws Exception {
		...
		BundleDeployHelper.deployBundle(new URI(wireMockRule.baseUrl()), bundleArchive, "bundle", "csdgroup",
				"cicsplex", "region", "username", "password".toCharArray(), true);
	}
	...
	public void testBundleDeployHelper_selfSignedNotValid() throws Exception {
		...
		try {
			BundleDeployHelper.deployBundle(new URI(wireMockRule.baseUrl()), bundleArchive, "bundle", "csdgroup",
					"cicsplex", "region", "username", "password".toCharArray(), false);
	...
	public void testBundleDeployHelper_invalidFile() throws Exception {
		...
		BundleDeployException e = assertThrows(BundleDeployException.class, () -> {
			BundleDeployHelper.deployBundle(new URI(wireMockRule.baseUrl()), bundleArchive, "bundle", "csdgroup",
					"cicsplex", "region", "username", "password".toCharArray(), true);
		});

		assertEquals("Bundle does not exist: 'invalid path'", e.getMessage());
	}
	...
	public void testBundleDeployHelper_invalidBundle() throws Exception {
		stubFor(post(urlEqualTo("/managedcicsbundles"))
				.willReturn(aResponse()
						...
						.withBody(
								"{\"message\":\"Some of the supplied parameters were invalid\",\"requestErrors\":{\"bundle\":\"Derived bundledir \\\""
										+ bundleFilePath + "\\\" didn't match the target BUNDDEF's bundle dir \\\""
										+ bundleFilePath + "\\\"\"}}")));
		...
		assertEquals(
				"Some of the supplied parameters were invalid:\n - bundle: Derived bundledir \"" + bundleFilePath
						+ "\" didn't match the target BUNDDEF's bundle dir \"" + bundleFilePath + "\"\n",
				e.getMessage());
	...
	public void testBundleDeployHelper_unauthenticated401() throws Exception {
		...
		assertEquals("Http response: HTTP/1.1 401 Unauthorized", e.getMessage());
	...
	public void testBundleDeployHelper_unauthenticated401_noContentType() throws Exception {
		...
		assertEquals("Http response: 401", e.getMessage());
